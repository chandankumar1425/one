...
async function getData(id) {
  let response = await fetch(
    `https://arcane-forest-74071.herokuapp.com/users/${id}`
  );
...
async function Searchuser() {
  ...
  let response =
    await fetch(`https://arcane-forest-74071.herokuapp.com/users?name=${name}
     `);
  let data = await response.json();
...
async function filterBatch() {
  ...
  console.log(batch);
  let response =
    await fetch(`https://arcane-forest-74071.herokuapp.com/users?batch_name=${batch}
    `);
  let data = await response.json();
  // console.log(data,"filter")
  displayData(data);
...
async function Changeprofesion(id){
    ...
    let response = await fetch(`https://arcane-forest-74071.herokuapp.com/users/${id}`,
    { method:"PATCH",
    headers: { 'Content-Type': 'application/json'},
    body: JSON.stringify({
